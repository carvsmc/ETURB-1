...
$(document).ready(function () {
    $.ajax({
      ...
      url: 'https://docs.google.com/spreadsheets/d/1MZwrgEyfcdFReZ218exg0YYvMno8xJYCm6iTbqpHf10/gviz/tq?tqx=out:csv&sheet=Sheet1',
      dataType: "text",
...
map.on('click', 'csvData', function(e) {
    ...
    var description =
      `<h3>` +
      e.features[0].properties.Tamanho_do_Buraco +
      `</h3>` +
      `<h4>` +
      `<b>` +
      `Address: ` +
      `</b>` +
      e.features[0].properties.Tipo_de_Cobertura +
      `</h4>` +
      `<h4>` +
      `<b>` +
      `Phone: ` +
...
const handleSubmit = (event) => {
    ...
    fetch('https://api.sheetmonkey.io/form/qudUvuQhRTABV7DuVE9Xod', {
        method: 'post',
        headers: {
